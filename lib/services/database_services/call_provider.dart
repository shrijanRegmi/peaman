...
import 'package:peaman/models/call_model.dart';
import 'package:peaman/models/user_model.dart';

class CallProvider {
  final PeamanCall? call;
  ...
  Future<bool?> checkAlreadyInCall() async {
    try {
      ...
      return _friendCallSnap?.docs.isNotEmpty ?? false;
    } catch (e) {
      print(e);
  ...
  Future _addToCollection() async {
    final _id = ChatHelper().getChatId(
      myId: appUser?.uid,
      friendId: friend?.uid,
    );
  ...
  Future _sendCalltoUsers(final DocumentReference? userRef) async {
    try {
      ...
      await _callRef?.set(_call?.toJson() ?? {});
      print('Success: Sending PeamanCall data to user ${userRef?.path}');
      return 'Success';
    } catch (e) {
      print(e);
      print('Error!!!: Sending PeamanCall data to user ${userRef?.path}');
      return null;
    }
  }
  ...
  Future endCall() async {
    try {
      ...
      final _friendCallRef =
          friend?.appUserRef?.collection('calls').doc(call?.id);
  ...
  PeamanCall? _callFromFirebase(
      final QuerySnapshot<Map<String, dynamic>> colSnap) {
    if (colSnap.docs.isNotEmpty) {
      return PeamanCall.fromJson(colSnap.docs.first.data());
    }
    return null;
  }

  // get cancelled call from firebase
  PeamanCall _cancelledCallFromFirebase(
      final DocumentSnapshot<Map<String, dynamic>> docSnap) {
    return PeamanCall.fromJson(docSnap.data() ?? {});
  }

  // stream of receiving call
  Stream<PeamanCall?> get receivingCall {
    return _ref
        .collection('users')
        .doc(appUser?.uid)
        .collection('calls')
  ...
  Stream<PeamanCall> get mainCall {
    return _ref
        .collection('users')
        .doc(appUser?.uid)
        .collection('calls')
